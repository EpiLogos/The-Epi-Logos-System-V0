...
class GradClass implements Grad {
  x: number;
  y: number;
  z: number;
...
class WavesEngine {
  ...
  private options: Required<WavesOptions>;
  ...
  constructor(container: HTMLElement, options: WavesOptions = {}) {
  ...
  private createCanvas() {
    ...
    this.canvas.className = 'absolute top-0 left-0 w-full h-full pointer-events-none z-[1]';
  ...
  private moved(point: Point, withCursor = true) {
...
export const WavesBackground: React.FC<WavesBackgroundProps> = ({
  ...
  const wavesEngineRef = useRef<WavesEngine | null>(null);

  useEffect(() => {
    if (containerRef.current) {
      wavesEngineRef.current = new WavesEngine(containerRef.current, {
        lineColor,
        waveSpeedX,
        waveSpeedY,
        waveAmpX,
    ...
    return () => {
      if (wavesEngineRef.current) {
        wavesEngineRef.current.destroy();
        wavesEngineRef.current = null;
      }
    };
