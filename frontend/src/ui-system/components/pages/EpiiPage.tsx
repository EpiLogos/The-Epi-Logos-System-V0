...
import { epiiContent } from '../../content/epii-content';
import { coordinate5_0Summary } from '../../content/coordinates/5-0-summary';
import { coordinate5_5Summary } from '../../content/coordinates/5-5-summary';
import { epiiViewContent } from '@/content/epii-view-on-epi-logos';
import { epiiModalCards, epiiSubnodeCards } from '../../content/epii-focus-cards';

export const EpiiPage: React.FC<{ coordinate?: string; autoExpand?: boolean }> = ({
  coordinate = '#5',
  autoExpand = false
}) => {
  ...
  // Logo text state
  const [logoText, setLogoText] = useState(epiiContent.collapsedLogoText);
  ...
  useEffect(() => {
    setLogoText(modalState.isModalExpanded ? epiiContent.expandedLogoText : epiiContent.collapsedLogoText);
  ...
  return (
    <div data-coordinate={coordinate}>
      <PageFadeIn>
        ...
        >
          {/* Epii's View Content - Behind sidebar, revealed when sidebar collapsed in modal collapsed state */}
          <div
            className={cn(
            ...
            {modalState.isModalExpanded && (
              ...
              >
                ...
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate5_0Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate5_0Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate5_0Summary.summary}
                  </p>
                </div>
                ...
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate5_5Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate5_5Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate5_5Summary.summary}
                  </p>
                </div>
              </div>
            )}
          ...
          >
            ...
            {(modalState.isModalExpanded || carouselVisible) && (
              ...
              >
                <FocusCards
                  cards={epiiModalCards}
                  className="w-full h-full"
                />
              </div>
            )}

            {/* Epii Image - Always clickable for modal close */}
            <ParamasivaImage
              src="/ui-system/epii-icon.png"
              alt="Epii Geometric Pattern"
      ...
      <SubnodesSection
        ...
        title="Subnodes"
        description="The four core aspects of the Epii synthesis layer, representing the integration of philosophical worldview, processing architecture, experiential design, and agent orchestration."
      />
