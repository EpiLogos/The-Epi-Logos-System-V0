...
import { naraContent } from '../../content/nara-content';
import { coordinate4_0Summary } from '../../content/coordinates/4-0-summary';
import { coordinate4_5Summary } from '../../content/coordinates/4-5-summary';
import { naraViewContent } from '@/content/nara-view-on-epi-logos';
import { naraModalCards, naraSubnodeCards } from '../../content/nara-focus-cards';

export const NaraPage: React.FC<{ coordinate?: string; autoExpand?: boolean }> = ({
  coordinate = '#4',
  autoExpand = false
}) => {
  ...
  // Logo text state
  const [logoText, setLogoText] = useState(naraContent.collapsedLogoText);
  ...
  useEffect(() => {
    setLogoText(modalState.isModalExpanded ? naraContent.expandedLogoText : naraContent.collapsedLogoText);
  ...
  return (
    <div data-coordinate={coordinate}>
      <PageFadeIn>
        ...
        >
          {/* Nara's View Content - Behind sidebar, revealed when sidebar collapsed in modal collapsed state */}
          <div
            className={cn(
          ...
          <div className={`flex flex-col flex-1 ${!modalState.isModalExpanded ? "justify-center" : ""}`}>
            ...
            {modalState.isModalExpanded && (
              ...
              >
                {/* Identity Matrix Box - #4-0 */}
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate4_0Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate4_0Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate4_0Summary.summary}
                  </p>
                </div>

                {/* Personal Gnosis Box - #4-5 */}
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate4_5Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate4_5Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate4_5Summary.summary}
                  </p>
                </div>
              </div>
            )}
          ...
          >
            ...
            {(modalState.isModalExpanded || carouselVisible) && (
              ...
              >
                <FocusCards
                  cards={naraModalCards}
                  className="w-full h-full"
                />
              </div>
            )}

            {/* Nara Image - Always clickable for modal close */}
            <ParamasivaImage
              src="/ui-system/nara-icon.png"
              alt="Nara Geometric Pattern"
      ...
      <SubnodesSection
        cards={naraSubnodeCards}
        title="Subnodes"
        description="The four core functional domains of the Nara personal interface, representing sympathetic medicine, divinatory frameworks, alchemical transformation, and lived contextual experience."
      />
