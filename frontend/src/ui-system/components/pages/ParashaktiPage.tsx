...
import { parashaktiContent } from '../../content/parashakti-content';
import { coordinate2_0Summary } from '../../content/coordinates/2-0-summary';
import { coordinate2_5Summary } from '../../content/coordinates/2-5-summary';
import { parashaktiViewContent } from '@/content/parashakti-view-on-epi-logos';
import { parashaktiModalCards, parashaktiSubnodeCards } from '../../content/parashakti-content';

export const ParashaktiPage: React.FC<{ coordinate?: string; autoExpand?: boolean }> = ({
  coordinate = '#2',
  autoExpand = false
}) => {
  ...
  // Logo text state
  const [logoText, setLogoText] = useState(parashaktiContent.collapsedLogoText);
  ...
  useEffect(() => {
    setLogoText(modalState.isModalExpanded ? parashaktiContent.expandedLogoText : parashaktiContent.collapsedLogoText);
  ...
  const handleSubsystemsClick = () => {
    transitionToSubsystems('parashakti');
  };
  ...
  return (
    <div data-coordinate={coordinate}>
      <PageFadeIn>
        ...
        >
          {/* Parashakti's View Content - Behind sidebar, revealed when sidebar collapsed in modal collapsed state */}
          <div
            className={cn(
          ...
          <div className={`flex flex-col flex-1 ${!modalState.isModalExpanded ? "justify-center" : ""}`}>
            ...
            {modalState.isModalExpanded && (
              ...
              >
                {/* Harmonic Well Box - #2-0 */}
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate2_0Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate2_0Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate2_0Summary.summary}
                  </p>
                </div>

                {/* Cosmic Orchestra Box - #2-5 */}
                <div className="flex-1 border border-gray-300 rounded-sm bg-white/50 p-6 flex flex-col">
                  <div className="mb-3">
                    <div className="text-xs font-mono text-gray-500 mb-1">{coordinate2_5Summary.coordinate}</div>
                    <h3 className="text-xs font-semibold text-gray-800 tracking-wide">
                      {coordinate2_5Summary.name.toUpperCase()}
                    </h3>
                  </div>
                  <p className="text-[10px] text-gray-600 leading-relaxed">
                    {coordinate2_5Summary.summary}
                  </p>
                </div>
              </div>
            )}
          ...
          >
            ...
            {(modalState.isModalExpanded || carouselVisible) && (
              ...
              >
                <FocusCards
                  cards={parashaktiModalCards}
                  className="w-full h-full"
                />
              </div>
            )}

            {/* Parashakti Image - Always clickable for modal close */}
            <ParamasivaImage
              src="/ui-system/parashakti-icon.png"
              alt="Parashakti Resonance Icon"
      ...
      <SubnodesSection
        cards={parashaktiSubnodeCards}
        title="Subnodes"
        description="The four core operations of the Parashakti vibrational stack, bridging modal harmonic tuning, cross-media translation, resonance diagnostics, and dramaturgical staging."
        logoSrc="/ui-system/parashakti-hex.png"
        logoAlt="Parashakti Hexagon"
      />
