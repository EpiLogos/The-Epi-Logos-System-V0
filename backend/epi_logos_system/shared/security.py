...
from backend.epi_logos_system.shared.exceptions import InvalidTokenError, UserNotFoundError, MFAError
...
def get_password_reset_service_dependency() -> PasswordResetService:
    ...
    from backend.epi_logos_system.users.services.audit_service import AuditService
    from shared.database import MongoDBClient
    import os
    user_service = get_user_service()
    mongodb_client = MongoDBClient()
    audit_service = AuditService(mongodb_client)
    jwt_secret = os.getenv("JWT_SECRET", "default-secret-change-in-production")
    return PasswordResetService(user_service.user_repository, audit_service, jwt_secret=jwt_secret)
