"""
Consolidated FastAPI routes for Google OAuth 2.0 authentication.

Merged from oauth_routes.py and oauth_exchange_routes.py for strategic consolidation.
Provides all OAuth flows including PKCE, account linking, and token exchange.
"""
...
from backend.epi_logos_system.auth.oauth.google_client import GoogleOAuthClientFactory
from backend.epi_logos_system.auth.oauth.account_linking_service import AccountLinkingService
...
from backend.epi_logos_system.users.models import UserRegistrationRequest
from backend.epi_logos_system.shared.container import get_user_service


logger = logging.getLogger(__name__)

# Create router with consolidated endpoints
...
async def google_oauth_callback(
    ...
    try:
        ...
        from backend.epi_logos_system.auth.jwt.handler import JWTHandler
...
@oauth_router.post("/google/exchange")
